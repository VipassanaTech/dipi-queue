...
if ( $argc <= 3)
{
   echo "Usage: php $argv[0] <Course-Date in yyyy-mm-dd> <dipi-centre-id> <vray-centre-id>\n";
   exit(1);
}

$course_date = $argv[1];
$centre_id = $argv[2];
$VRAY_CENTRE_ID = $argv[3];

include_once("constants.inc");
include_once("constants-vray.inc");

$attended = "select (FirstName + ' ' + LastName) as Name, Email from RegApplicantsMAster
...
where coursemaster.centreid =$VRAY_CENTRE_ID and RegApplicantsCourseDetail.Attended = 1 and coursemaster.startdate='".$course_date."'";
...
join RegApplicantsCourseDetail on RegApplicantsMAster.RegApplicantID =  RegApplicantsCourseDetail.RegApplicantID left join coursemaster on RegApplicantsCourseDetail.CourseID = coursemaster.courseid 
where coursemaster.centreid =$VRAY_CENTRE_ID and coursemaster.startdate='".$course_date."'";
...
$result = mssql_query($confirmed) or die("COuld not exec sql - ".mssql_error($link));
$count = 0;
...
while($row = mssql_fetch_array($result))
{
   ...
   if ( $ins['a_old'] )
   {
	...
	$ins_c['ac_10d'] = $row['TotalCoursesSat'];
	$ins_c['ac_service'] = $row['TotalCoursesServed'];
